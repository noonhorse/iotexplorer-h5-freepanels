...
export function RoundDashboard(props: DashboardProps) {
  ...
  const currentAngle = (() => {
    //进度
    let progress = value / (maxValue - minValue);
  ...
  const currentOpacity: number = (() => {
    return Number(((1 - 0.15) / ((currentAngle - startAngle) / step)).toFixed(2))
  })()
  ...
  const lineArray = () => {
    ...
    for (let i = startAngle; i <= endAngle; i += step) {
      ...
      // 0 刻度线跟svg外层距离
      let x = (r1 + (r1 - 0) * Math.cos(i * Math.PI / 180));
      let y = (r1 + (r1 - 0) * Math.sin(i * Math.PI / 180));

      let x2 = (r1 + (r2 - 0) * Math.cos(i * Math.PI / 180));
      let y2 = (r1 + (r2 - 0) * Math.sin(i * Math.PI / 180));
  ...
  const renderRoundScale = () => {
    ...
    for (let i = startAngle; i <= endAngle; i += 14) {
      ...
      // 0 刻度线跟svg外层距离
      let x = (r1 + (r1 - 130) * Math.cos(i * Math.PI / 180));
      let y = (r1 + (r1 - 130) * Math.sin(i * Math.PI / 180));
  ...
  const ellipse2path = (sx: number, sy: number, ex: number, ey: number) => {
      ...
      let angle = startAngle + (currentAngle - startAngle)/2

      let x = (radius + (radius - 104) * Math.cos(angle * Math.PI / 180));
      let y = (radius + (radius - 104) * Math.sin(angle * Math.PI / 180));

      //path 属性
      var descriptions = [
        'M', sx, sy, 
        // 'Q', x, y, ex, ey
        'A', x, y, angle, largeArcFlag, 1, ex, ey,
  ...
  const renderIndicator = () => {
    ...
    // 124指示球距离圆形边的距离 
    let startX = (radius + (radius - 104) * Math.cos(startAngle * Math.PI / 180));
    let startY = (radius + (radius - 104) * Math.sin(startAngle * Math.PI / 180));
    ...
    let dpath = ellipse2path(startX, startY, indicator.x, indicator.y)
    return (
      <>
        <path d={dpath}
          stroke="#ccc" strokeWidth="0"
          fill="none"
          id="myPath"
          />
        <circle
    ...
    >
      <defs>
        { centerCicle.color === 'url(#gradient)' ?
          <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%" gradientTransform="rotate(49)">
            <stop offset="0%" stopColor={centerCicle.startColor ? centerCicle.startColor : '#527DF4'}/>
            <stop offset="100%" stopColor={centerCicle.endColor ? centerCicle.endColor : '#044DFF'} />
          </linearGradient> : null
        }
